    ...
    def post(self, request):
        try:
            ...
            payload = {
                ...
                'expiry_time': (datetime.now() + timedelta(seconds=1)).timestamp()
            }
            print(payload)
            ...
            if recovered_address.lower() == user_address.lower():
                profile = Profile.objects.get_or_create(user_address=user_address)[0]
    ...
    def post(self, request):
        try:
            ...
            if (decoded_token["expiry_time"]>datetime.now().timestamp()):
                return JsonResponse({'message': 'Token is valid', 'data': decoded_token})
